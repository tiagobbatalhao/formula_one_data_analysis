...
class FourierFit:
    ...
    def _get_basis_function(self, val: float) -> np.ndarray:
        ...
        arr[0] = 1.0
        arr[1::2] = [np.cos(val * k * self._twopi) for k in range(1, 1 + self.max_degree)]
        arr[2::2] = [np.sin(val * k * self._twopi) for k in range(1, 1 + self.max_degree)]
        return arr
    ...
    def fit(self, X: Union[np.ndarray, List[float]], y: Union[np.ndarray, List[float]]) -> 'FourierFit':
    ...
    def fit_predict(self, X: Union[np.ndarray, List[float]], y: Union[np.ndarray, List[float]]) -> np.ndarray:
...
def fit_map_by_time(df_position: pd.DataFrame, max_degree: int) -> Dict[str, FourierFit]:
    ...
    """
    required_columns = ["timing_from_lap", "time_lap", "coordinate_x", "coordinate_y", "coordinate_z"]
...
def correct_fit_by_distance(
    ...
    data_by_time = np.column_stack([
        fitting_by_time[k].predict(predict_parameter)
        for k in ["x", "y", "z"]
    ])
...
def adjust_starting_point(fitting: Dict[str, FourierFit], df_pos: pd.DataFrame) -> float:
...
def format_output(
    ...
    output_f = np.column_stack([
        fitting[k].predict(call_parameter)
        for k in ["x", "y", "z"]
    ])
    output_der = np.column_stack([
        fitting[k].predict_d1(call_parameter)
        for k in ["x", "y", "z"]
    ])
    ...
    # Calculate distances
    diff_coordinate_x = np.diff(np.append(save["coordinate_x"], save["coordinate_x"][0]))
    diff_coordinate_y = np.diff(np.append(save["coordinate_y"], save["coordinate_y"][0]))
...
def main(session_ids: List[str], best_laps: int, max_degree: int, predict_size: int) -> Dict:
    ...
    df_laps_keep = (
        df_laps[(df_laps["is_accurate"])]
        .sort_values(by=["time_lap"])
        .iloc[:best_laps]
    )
    
    idx = ["year", "session_id", "driver_number", "lap_number"]
    df_pos_keep = df_pos.merge(
        df_laps_keep[idx + ["time_lap"]], 
        on=idx, 
        how="inner"
    )
    ...
    output["data"].to_parquet(f"circuit_map_{session_ids_str}.parquet", index=False)
    return output
